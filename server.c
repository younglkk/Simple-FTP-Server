...
void server(int port)
{
  ...
  memset(&client_address, 0, len); // Andy Su
  int connection, pid, bytes_read;

  while(1){
    if ((connection = accept(sock, (struct sockaddr*)&client_address, &len)) < 0) {
      perror("accept()");
      exit(1);
    }
...
int accept_connection(int socket)
{
  ...
  printf("finish accept_connection setting\n");
...
void getip(int sock, int *ip)
{
  ...
  printf("Start getip:sscanf()\n");
  sscanf(host,"%d.%d.%d.%d",&ip[0],&ip[1],&ip[2],&ip[3]); 
  printf("End getip:sscanf()\n");
}
